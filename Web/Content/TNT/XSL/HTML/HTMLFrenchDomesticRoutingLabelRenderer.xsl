...
<div id="boxFR">
  ...
  <div id="customerReferenceBarcodeFR" name="customerReferenceBarcodeFR">
      <img width="220px" height="13px">
          <xsl:if test="string-length(barcodeForCustomer) > 0">
			  <xsl:attribute name="src">
			      <xsl:value-of select="concat($barcode_url,barcodeForCustomer)" />
              </xsl:attribute>
          </xsl:if> 
      </img>
  </div>
  ...
  <div id="contactNameFR">
      <span id="contactNameHeaderFR">Contact Name: </span>
  ...
  <div id="contactPhoneFR">
      <span id="contactPhoneHeaderFR">Contact Phone: </span>
  ...
  <div id="postcodeHeaderFR">Code Postale /
      <br />
      Code Satellite
  </div>
  <div id="postcodeFR">
...
<xsl:template name="FormatDateFrenchDomestic">
    ...
    <xsl:choose>
        ...
        <xsl:when test="$mo = '2' or $mo = '02'">Fev</xsl:when>
        <xsl:when test="$mo = '3' or $mo = '03'">Mars</xsl:when>
